...
## exactly one full rotation (TAU radians) over `flight_seconds` — a
## "flush" landing. Below 90 spins slower (under-rotates by landing);
## above 90 spins faster (over-rotates).
static func spin_rate(gauge_degrees: float, flight_seconds: float) -> float:
	return (TAU / flight_seconds) * (gauge_degrees / 90.0)
