...
**Spin axis matters (found and fixed once, worth knowing about):** the karttu's angular velocity must be applied about the *launch direction itself* (a barrel roll along the flight path), not `dir.cross(Vector3.UP)` — for a straight-ahead throw that axis coincides with the karttu's own length (since it rests with its length along world X), so spinning about it is just a rolling-pin roll that never changes orientation relative to the ground. Only spinning about the travel axis actually alternates the karttu between lying flat and standing on end, which is what makes "release at 90° to land flush" physically meaningful. Verified numerically (angular velocity axis/magnitude match `SpinCalculator` exactly) with a throwaway headless script; the resulting *feel* (does a flush hit visibly sweep more kyykkä than a bad-angle one) hasn't been checked visually.
