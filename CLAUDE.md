...
**Spin axis matters (found and fixed once, worth knowing about):** the karttu's angular velocity must be applied about the *horizontal aim direction* (a level barrel roll along the flight path), not `dir.cross(Vector3.UP)` — for a straight-ahead throw that axis coincides with the karttu's own length (since it rests with its length along world X), so spinning about it is just a rolling-pin roll that never changes orientation relative to the ground. Only spinning about the (horizontal) travel axis actually alternates the karttu between lying flat and standing on end, which is what makes "release at 90° to land flush" physically meaningful.

**Rotation is driven kinematically while airborne, not left to the physics solver:** setting `angular_velocity` once (or even re-asserting it every physics frame) still let the karttu's spin visibly wobble off-axis — measured directly: `angular_velocity.y` reached ~1.85 rad/s within a single physics step even with a per-frame reset, i.e. real solver-computed torque/precession, not float drift. `ThrowController` now overrides `global_transform.basis` every `_physics_process` tick with an exact analytic rotation (`_lock_start_basis.rotated(_lock_axis, _lock_rate * _lock_elapsed)`) for as long as `_locking_spin` is true, guaranteeing zero wobble regardless of what the solver would have computed. This releases back to normal physics (`_locking_spin = false`) on the karttu's first real contact — but `get_contact_count()` can keep reporting the *pre-launch* resting-on-ground contact for a few frames right after unfreezing, so contacts are ignored for `lock_grace_seconds` (0.15s) after launch; without that grace period the lock released after 3-4 frames instead of the ~54 (≈0.9s, the full flight) it should. Verified numerically with a throwaway headless script (zero `angular_velocity.y` throughout the locked phase, release timing matching actual flight duration); the resulting *feel* (does a flush hit visibly sweep more kyykkä than a bad-angle one) still hasn't been checked visually.
